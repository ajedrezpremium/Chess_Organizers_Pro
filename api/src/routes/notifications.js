...
router.get('/', authenticate, async (req, res) => {
  ...
  const unreadCount = (await db.prepare(`SELECT COUNT(*) as c FROM notifications WHERE user_id = ? AND read = 0`).get(req.user.id)).c;
...
router.patch('/:id/read', authenticate, async (req, res) => {
  const db = getDb();
  const n = await db.prepare(`SELECT id FROM notifications WHERE id = ? AND user_id = ?`).get(req.params.id, req.user.id);
...
router.get('/settings', authenticate, async (req, res) => {
  const db = getDb();
  let s = await db.prepare(`SELECT * FROM user_notification_settings WHERE user_id = ?`).get(req.user.id);
  if (!s) {
    await db.prepare(`INSERT INTO user_notification_settings (user_id) VALUES (?)`).run(req.user.id);
    s = await db.prepare(`SELECT * FROM user_notification_settings WHERE user_id = ?`).get(req.user.id);
  }
  delete s.id;
  delete s.user_id;
...
router.patch('/settings', authenticate, async (req, res) => {
  ...
  await db.prepare(`INSERT INTO user_notification_settings (user_id) VALUES (?) ON CONFLICT(user_id) DO UPDATE SET ${sets.join(', ')}`).run(req.user.id, ...vals.slice(0, -1));
  await db.prepare(`UPDATE user_notification_settings SET ${sets.join(', ')} WHERE user_id = ?`).run(...vals);
  res.json({ ok: true });
});
