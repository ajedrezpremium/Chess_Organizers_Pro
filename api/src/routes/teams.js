...
router.post('/tournaments/:tid/teams', async (req, res) => {
  const db = getDb();
  ...
  const result = await db.prepare('INSERT INTO teams (tournament_id, name, short_name) VALUES (?, ?, ?)').run(req.params.tid, name.trim(), (short_name || name.trim().slice(0, 3).toUpperCase()));
  const team = await db.prepare('SELECT * FROM teams WHERE id = ?').get(result.lastInsertRowid);
  res.status(201).json(team);
});

// GET /tournaments/:tid/teams — listar equipos con miembros
router.get('/tournaments/:tid/teams', async (req, res) => {
  const db = getDb();
  const teams = await db.prepare('SELECT * FROM teams WHERE tournament_id = ? ORDER BY name ASC').all(req.params.tid);
  for (const team of teams) {
    team.members = await db.prepare(`
...
router.post('/teams/:id/members', async (req, res) => {
  ...
  const tp = await db.prepare('SELECT id FROM tournament_players WHERE id = ? AND tournament_id = ?').get(tournament_player_id, team.tournament_id);
  if (!tp) return res.status(400).json({ error: 'Jugador no pertenece al torneo' });

  try {
    const result = await db.prepare('INSERT INTO team_members (team_id, tournament_player_id, board_number) VALUES (?, ?, ?)').run(req.params.id, tournament_player_id, board_number ?? 1);
...
router.get('/tournaments/:tid/team-standings', async (req, res) => {
  const db = getDb();
  const teams = await db.prepare('SELECT * FROM teams WHERE tournament_id = ? ORDER BY name ASC').all(req.params.tid);
