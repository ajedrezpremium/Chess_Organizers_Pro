...
export function usePairings(
  ...
  const currentTRF = useMemo(() => {
    if (!tournamentConfig) return null;
      const system = tournamentConfig?.system ?? 'dutch';
      const typeMap = { roundrobin: 'R', burstein: 'S-A', dubov: 'S-X', dutch: 'S' };
      const config = {
        ...
        tournamentTypeCode: typeMap[system] ?? 'S',
        chiefArbiter:       tournamentConfig.arbiter     ?? '',
        nRounds:            totalRounds,
        tiebreaks:          tournamentConfig.tiebreaks   ?? DEFAULT_TIEBREAK_ORDER,
        extendedType:       system === 'roundrobin' ? `IND RR ${totalRounds}R` : `IND SWISS ${totalRounds}R`,
      };
  ...
  const generateNextRound = useCallback(async () => {
    ...
    try {
      const result = await pairingEngine.pairNextRound({
        trf:     currentTRF,
        system:  tournamentConfig?.system ?? 'dutch',
