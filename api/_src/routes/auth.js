...
router.get('/me', authenticate, async (req, res) => {
  try {
    const db = getDb();
    const user = await db.prepare('SELECT id, email, name, role, federation, fide_id, verified, created_at FROM users WHERE id = ?').get(req.user.id);
...
router.post('/push-token', authenticate, async (req, res) => {
  try {
    ...
    await db.prepare(`UPDATE users SET push_token = ?, push_platform = ? WHERE id = ?`).run(token, platform || '', req.user.id);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
