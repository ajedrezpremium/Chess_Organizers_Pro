...
async function getLocalKnowledge() {
  ...
  try {
    const mod = await import('../data/fide-laws-2023.js');
    localKnowledge = mod;
    return mod;
  } catch {
    return null;
...
function buildFallbackAnswer(question) {
  ...
  if (q.includes('pieza tocada') || q.includes('tocar') || q.includes('tocada') || q.includes('touched')) {
    return { answer: 'Artículo 4.3: Si el jugador con el turno toca una pieza propia, debe moverla. Si toca una pieza del oponente, debe capturarla (si es posible). Artículo 4.6: Si es imposible identificar qué pieza se tocó primero, se considera que es la pieza propia del jugador y no la del oponente.', article: '4.3', title: 'Pieza tocada', confidence: 'high', related: [] };
  }
