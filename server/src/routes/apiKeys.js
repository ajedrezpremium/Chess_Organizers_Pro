...
router.get('/', authenticate, (req, res) => {
  const db = getDb();
  const keys = db.prepare('SELECT id, name, key, last_used_at, created_at FROM api_keys WHERE user_id = ? ORDER BY created_at DESC').all(req.user.id);
  res.json(keys);
});
...
router.post('/', authenticate, (req, res) => {
  ...
  const result = db.prepare('INSERT INTO api_keys (user_id, name, key) VALUES (?, ?, ?)').run(req.user.id, name.trim(), hashedKey);
