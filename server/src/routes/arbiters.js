...
router.get('/search-users', authenticate, (req, res) => {
  ...
  const users = await db.prepare("SELECT id, name, email, role FROM users WHERE email LIKE ? OR name LIKE ? LIMIT 20").all(`%${q}%`, `%${q}%`);
  res.json(users);
});
...
router.get('/tournaments/:id', authenticate, (req, res) => {
  ...
  for (const r of rounds) {
    ...
    for (const p of r.pairings) {
      if (p.white_id) {
        const w = await db.prepare('SELECT tp.id, pl.name, pl.last_name FROM tournament_players tp JOIN players pl ON pl.id = tp.player_id WHERE tp.id = ?').get(p.white_id);
        if (w) { p.white_name = w.name; p.white_last = w.last_name; }
      }
      if (p.black_id) {
        const b = await db.prepare('SELECT tp.id, pl.name, pl.last_name FROM tournament_players tp JOIN players pl ON pl.id = tp.player_id WHERE tp.id = ?').get(p.black_id);
...
router.patch('/rounds/:rid/result', authenticate, (req, res) => {
  ...
  await db.prepare('UPDATE pairings SET result = ? WHERE id = ? AND round_id = ?').run(result, pairing_id, req.params.rid);
  res.json({ ok: true });
});
...
router.post('/players/:tpId/check-in', authenticate, (req, res) => {
  ...
  await db.prepare('UPDATE tournament_players SET checked_in = 1 WHERE id = ?').run(req.params.tpId);
...
router.get('/tournaments/:id/arbiters', authenticate, (req, res) => {
  ...
  const arbiters = await db.prepare(`
...
router.post('/tournaments/:id/arbiters', authenticate, (req, res) => {
  ...
  const user = await db.prepare('SELECT id, name, email FROM users WHERE id = ?').get(user_id);
  if (!user) return res.status(404).json({ error: 'Usuario no encontrado' });

  try {
    await db.prepare('INSERT INTO tournament_arbiters (tournament_id, user_id) VALUES (?, ?)').run(req.params.id, user_id);
...
router.delete('/tournaments/:id/arbiters/:userId', authenticate, (req, res) => {
  ...
  await db.prepare('DELETE FROM tournament_arbiters WHERE tournament_id = ? AND user_id = ?').run(req.params.id, req.params.userId);
  res.json({ ok: true });
});
