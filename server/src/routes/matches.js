...
router.post('/', (req, res) => {
  ...
  const home = db.prepare('SELECT id, tournament_id FROM teams WHERE id = ?').get(home_team_id);
  const away = db.prepare('SELECT id, tournament_id FROM teams WHERE id = ?').get(away_team_id);
  ...
  const match = db.prepare('SELECT * FROM team_matches WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`UPDATE team_matches SET ${updates.join(', ')}, updated_at = datetime('now') WHERE id = ?`).run(...params);
  res.json(db.prepare('SELECT * FROM team_matches WHERE id = ?').get(req.params.id));
});

// DELETE /matches/:id
router.delete('/:id', (req, res) => {
  const db = getDb();
  const r = db.prepare('DELETE FROM team_matches WHERE id = ? AND created_by = ?').run(req.params.id, req.user.id);
...
router.put('/:mid/pairings/:pid', (req, res) => {
  const db = getDb();
  const { result } = req.body;
  if (!result) return res.status(400).json({ error: 'Resultado requerido' });
  db.prepare('UPDATE match_pairings SET result = ? WHERE id = ?').run(result, req.params.pid);

  // Recalcular scores del match
  const pairings = db.prepare('SELECT result FROM match_pairings WHERE team_match_id = ?').all(req.params.mid);
  ...
  db.prepare('UPDATE team_matches SET home_score = ?, away_score = ? WHERE id = ?').run(homeScore, awayScore, req.params.mid);

  res.json(db.prepare('SELECT * FROM match_pairings WHERE id = ?').get(req.params.pid));
});

// DELETE /matches/:mid/pairings/:pid
router.delete('/:mid/pairings/:pid', (req, res) => {
  const db = getDb();
  db.prepare('DELETE FROM match_pairings WHERE id = ? AND team_match_id = ?').run(req.params.pid, req.params.mid);
  res.json({ ok: true });
});
