...
router.get('/:id', (req, res) => {
  ...
  league.participants = db.prepare(`
...
router.put('/:id', (req, res) => {
  const db = getDb();
  const league = db.prepare('SELECT * FROM leagues WHERE id = ? AND created_by = ?').get(req.params.id, req.user.id);
  if (!league) return res.status(404).json({ error: 'Liga no encontrada' });
  ...
  db.prepare(`UPDATE leagues SET ${updates.join(', ')}, updated_at = datetime('now') WHERE id = ?`).run(...params);
...
router.delete('/:id', (req, res) => {
  const db = getDb();
  const r = db.prepare('DELETE FROM leagues WHERE id = ? AND created_by = ?').run(req.params.id, req.user.id);
  if (!r.changes) return res.status(404).json({ error: 'Liga no encontrada' });
  res.json({ ok: true });
});

// POST /leagues/:id/tournaments — asignar torneo a liga
router.post('/:id/tournaments', (req, res) => {
  const db = getDb();
  const league = db.prepare('SELECT * FROM leagues WHERE id = ? AND created_by = ?').get(req.params.id, req.user.id);
  if (!league) return res.status(404).json({ error: 'Liga no encontrada' });
  const { tournament_id, weight } = req.body;
  if (!tournament_id) return res.status(400).json({ error: 'Torneo requerido' });
  const t = db.prepare('SELECT id FROM tournaments WHERE id = ?').get(tournament_id);
  if (!t) return res.status(404).json({ error: 'Torneo no encontrado' });
  const maxRound = db.prepare('SELECT COALESCE(MAX(round_number),0) as r FROM league_tournaments WHERE league_id = ?').get(req.params.id);
  try {
    db.prepare('INSERT INTO league_tournaments (league_id, tournament_id, round_number, weight) VALUES (?, ?, ?, ?)').run(req.params.id, tournament_id, maxRound.r + 1, weight || 1.0);
    res.status(201).json({ ok: true });
  } catch { res.status(409).json({ error: 'El torneo ya está en la liga' }); }
});

// DELETE /leagues/:lid/tournaments/:tid
router.delete('/:lid/tournaments/:tid', (req, res) => {
  const db = getDb();
  db.prepare('DELETE FROM league_tournaments WHERE league_id = ? AND tournament_id = ?').run(req.params.lid, req.params.tid);
  res.json({ ok: true });
});

// POST /leagues/:id/calculate-standings — recalcular clasificación
router.post('/:id/calculate-standings', (req, res) => {
  const db = getDb();
  const league = db.prepare('SELECT * FROM leagues WHERE id = ?').get(req.params.id);
  if (!league) return res.status(404).json({ error: 'Liga no encontrada' });
