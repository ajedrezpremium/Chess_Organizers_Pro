...
router.get('/plans', (req, res) => {
  const db = getDb();
  const plans = db.prepare('SELECT * FROM membership_plans ORDER BY price_usd ASC').all();
  res.json(plans);
});
...
router.get('/my', authenticate, (req, res) => {
  ...
  const activeCount = db.prepare("SELECT COUNT(*) as c FROM tournaments WHERE created_by = ? AND status = 'active'").get(req.user.id).c;
...
router.post('/subscribe', authenticate, (req, res) => {
  ...
  const plan = db.prepare('SELECT * FROM membership_plans WHERE slug = ?').get(plan_slug);
  if (!plan) return res.status(404).json({ error: 'Plan no encontrado' });

  // Free plan: direct subscription (no payment)
  if (plan.price_usd === 0) {
    db.prepare("UPDATE user_memberships SET status = 'cancelled' WHERE user_id = ? AND status = 'active'").run(req.user.id);
    db.prepare('INSERT INTO user_memberships (user_id, plan_id, status) VALUES (?, ?, ?)').run(req.user.id, plan.id, 'active');
