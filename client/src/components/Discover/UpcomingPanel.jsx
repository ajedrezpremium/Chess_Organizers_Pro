...
export default function UpcomingPanel({ items = [], live = false }) {
  ...
  return (
    <Card>
      <CardHeader title="📅 Próximos 30 días" subtitle="Avance de los más importantes · tipo 2700 Live"
        action={<span className="text-[10px] text-gray-400">{list.length} eventos{live ? ' · ● LIVE' : ''}</span>} />
      ...
      <div className="p-4 grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
        {list.map((t) => {
          const web = t.officialUrl || t.broadcastUrl || '';
          return (
            <div key={t.id} className="border border-gray-200 dark:border-fide-700 rounded-xl p-3 hover:shadow-md transition">
              {web ? (
                <a href={web} target="_blank" rel="noreferrer" title={`${t.name} — web oficial`}
                  className="font-bold text-xs dark:text-white truncate block hover:text-amber-600 dark:hover:text-amber-400 hover:underline">{t.name}</a>
              ) : (
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <span className="ml-auto flex items-center gap-2">
                  {web && (
                    <a href={web} target="_blank" rel="noreferrer" title="Web oficial del evento"
                      className="text-[11px] font-semibold text-amber-600 dark:text-amber-400 hover:underline">🌐 Website</a>
                  )}
                  <button className="text-[11px] font-semibold text-amber-600 dark:text-amber-400 hover:underline">🔔 Radar</button>
                </span>
              </div>
            </div>
          );
