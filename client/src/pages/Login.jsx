  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-fide-900 to-fide-700 p-4">
      <div className="bg-white dark:bg-fide-800 rounded-xl shadow-2xl p-8 w-full max-w-sm">
        ...
        <h1 className="text-2xl font-bold text-center mb-6 dark:text-white">Chess Organizers Pro</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-fide-200 mb-1">Email</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-fide-200 mb-1">Contraseña</label>
          ...
          <button type="submit" disabled={loading} className="w-full bg-fide-700 hover:bg-fide-800 disabled:opacity-50 text-white font-semibold py-2 rounded-lg transition">{loading ? 'Ingresando...' : 'Ingresar'}</button>
        </form>
        <p className="text-center text-sm text-gray-500 dark:text-fide-300 mt-4">
          ¿No tienes cuenta? <Link to="/register" className="text-fide-600 dark:text-fide-300 hover:underline">Regístrate</Link>
        </p>
      </div>
    </div>
  );
