...
function parsePlayerLine(line, warnings) {
  ...
  const roundResults = [];
  let col = 90; // índice 0-based
  while (col + 8 <= line.length) {
    const chunk     = line.substring(col, col + 8);
    const oppRank   = parseInt(chunk.substring(1, 5).trim(), 10) || null;
    const colorChar = chunk[5]?.toLowerCase() ?? '-';
    const resultChar = chunk[7] ?? '-';
...
export function serializeTRF(config, players, rounds) {
  ...
  for (const [playerRank, player] of sortedPlayers.entries()) {
    ...
    const line = [
      ...
      ' ',
      fideid,
      '           ',  // fecha nacimiento (no obligatoria)
      pts,
      ' ',
      rankStr,
      roundStr,
